...
function customAssign(...objects) {
  if(objects.length > 2) {
    ...
    let result =  Object.assign(result, ...objects);
...
function customObjectFromEntries(entries) {
  if(!Array.isArray(entries)) {
    ...
    const arEnt = entries.map(([key, value]) => {
   ...
   return Object.fromEntries(arEnt);
  }
...
Object.freeze(obj1);
    ...
    return "повідомлення про помилку, якщо об'єкт не було заморожено"
  }
...
function keysAndValues(obj) {
  if(typeof obj !== 'object') {
    ...
    let keysAnd = Object.keys(obj);
    let keyVal = Object.values(obj);
   let result = [keysAnd, keyVal];
   return result;
  }
  }
...
function checkOwnership(obj1, obj2, key) {
  if(obj1.hasOwnProperty(key) || obj2.hasOwnProperty(rey)) {
...
function getObjectValuesSum(obj) {
  if(typeof obj !== 'object') {
   ...
   return Object.values(obj).reduce((key,val) => key + val);
  }
...
function convertArrayToObj(arr) { 
  if(!Array.isArray(arr)) {
    ...
    let obj3 = {};
    // Це жахливе завдання і я його робила 100% не сама, ще й навряд зрозуміла....
  for(let b = 0; b < arr.length; b++ ) {
    const [key, value] = arr[b];
    if(key in obj3) {
      console.log (`У масиві є дубльований ключ: ${key}`);
    }
    obj3[key] = value;
  }
  Object.fromEntries(Object.entries(obj3));
  return obj3;
  }
